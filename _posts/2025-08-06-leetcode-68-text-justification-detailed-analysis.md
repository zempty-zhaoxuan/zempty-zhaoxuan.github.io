...
*   `len` 记录了从 `words[i]` 到 `words[j-1]` 这些单词的纯字符总长度（**不包含任何空格**）。

循环条件 `len + words[j].length() + (j - i - 1) < maxWidth` 是这道题最关键的部分之一。我们来分解它：

*   `len`: 已经放入行的单词的总长度（纯字符，不含空格）。
*   `words[j].length()`: 准备要放入的新单词的长度。
*   `(j - i - 1)`: **这部分是理解的关键**。这个值表示的是**当前已经在行内的单词之间的间隙数量**。注意，这里计算的是 `words[i]` 到 `words[j-1]` 之间的间隙，**不包括** `words[j-1]` 和 `words[j]` 之间的空格。
    *   如果当前行已有 `j - i` 个单词（从 `words[i]` 到 `words[j-1]`），它们之间有 `(j - i) - 1` 个间隙。
    *   例如：2个单词之间有1个间隙，3个单词之间有2个间隙。
    *   为了满足文本对齐的基本要求，每个间隙**至少需要一个空格**。
    *   所以 `(j - i - 1)` 代表了**当前行内已有单词之间所需的最少空格数**。

**完整条件的含义：**
`len + words[j].length() + (j - i - 1)` 表示：
- 当前行已有单词的总长度 (`len`)
- 加上新单词的长度 (`words[j].length()`)
- 加上当前行内单词之间的最少空格 (`j - i - 1`)
- **注意：这里没有加上 `words[j-1]` 和 `words[j]` 之间的空格，因为如果 `words[j]` 能放入，它们之间至少需要1个空格，这会在下一次循环中通过 `j - i - 1` 体现出来。**

实际上，当我们尝试放入 `words[j]` 时，完整的长度应该是：
`len + words[j].length() + (j - i)` （包括 `words[j-1]` 和 `words[j]` 之间的1个空格）

但代码写的是 `(j - i - 1)`，这是因为在下一次循环更新 `len` 时，只累加了单词长度，没有累加空格。这样的写法等价于：
- 当前检查：`len + words[j].length() + (j - i - 1) < maxWidth`
- 等价于：`len + words[j].length() + (j - i - 1) + 1 <= maxWidth`
- 即：`len + words[j].length() + (j - i) <= maxWidth`
...
2.  **尝试放入 `words[1]` ("is")**:
    *   `j=1`
    *   `len` (当前单词总长) = 4 (只有 "This")
    *   `words[1].length()` = 2 ("is")
    *   当前行内单词间隙数 `j - i - 1` = `1 - 0 - 1` = 0 (因为只有1个单词 "This"，没有间隙)
    *   总长度 = `4 + 2 + 0 = 6`. `6 < 20`，成立。
    *   可以放入。更新 `len`: `len = 4 + 2 = 6`。`j` 变为 2。
3.  **尝试放入 `words[2]` ("an")**:
    *   `j=2`
    *   `len` (当前单词总长) = 6 ("This" + "is")
    *   `words[2].length()` = 2 ("an")
    *   当前行内单词间隙数 `j - i - 1` = `2 - 0 - 1` = 1 ("This" 和 "is" 之间有1个间隙)
    *   总长度 = `6 + 2 + 1 = 9`. `9 < 20`，成立。
...
4.  **尝试放入 `words[3]` ("example")**:
    *   `j=3`
    *   `len` (当前单词总长) = 8 ("This" + "is" + "an")
    *   `words[3].length()` = 7 ("example")
    *   当前行内单词间隙数 `j - i - 1` = `3 - 0 - 1` = 2 ("This"-"is" 和 "is"-"an" 两个间隙)
